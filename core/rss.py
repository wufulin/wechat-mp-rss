...
class RSS:
    ...
    def add_logo_prefix_to_urls(self, text: str) -> str:
        """给微信图片 URL 添加 /static/res/logo/ 代理前缀。
        ...
        import re
        try:
            pattern = re.compile(r'(<img[^>]*src=["\'])(?!\/static\/res\/logo\/)(https?:\/\/(?:mmbiz\.qpic\.cn|mmbiz\.qlogo\.cn|mmecoa\.qpic\.cn|wx\.qlogo\.cn|thirdwx\.qlogo\.cn)\/[^"\']*)', re.IGNORECASE)
    ...
    def generate_by_template(self,rss_list: dict, template: str, title: str = "WeRSS",link: str = "https://github.com/letuswerss/werss",description: str = "RSS频道",language: str = "zh-CN",image_url:str=""):
